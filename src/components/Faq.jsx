...
const FaqCard = ({ question, answer, index }) => {
  ...
  return (
    <div className={` flex flex-col min-w-[200px] w-full sm:w-[49%] gap-6 my-2 sm:my-4 rounded-md  border-[1px] border-[#cacaca] text-black  md:gap-4 cursor-pointer `} onClick={() => setActiveFaq(!activeFaq)}>
      <div className={`${styles.h4} p-2 md:p-2 text-primary_10 flex justify-between items-center`}><p>{question}</p> <span className="p-1 text-xl md:text-2xl lg:text-2xl xl:text-2xl font-medium leading-normal rounded-full">{activeFaq?"-":"+"}</span> </div> 
