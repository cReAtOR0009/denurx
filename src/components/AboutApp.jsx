...
const AboutApp = () => {
  ...
  return (
    <section
     id="about" className={`${styles.container} px-4 py-4 md:px-10 md:py-10 lg:p-20 xl:p-20 text-black`} //bg-primary_200
    >
      <div className="flex flex-col md:flex-row justify-between item flex-wrap  gap-4 p-2   md:p-8 md:rounded-[50px] border-[5px] rounded  md:border-[10px] border-primary border-solid">
        <div className="order-2 sm:order-1 static p-0 md:p-4 flex gap-2 md:gap-4 md:relative  md:grid place-items-center  flex-1 w-auto flex-wrap ">
          {aboutApp.images.map((image, index) => {
            ...
            return (
              <ImageCard key={index} index={index} image={image} position={position}/>
            );
          })}
        </div>
        <div className="order-1 sm:order-2 flex flex-col flex-1 gap-6 sm:gap-10">
          ...
          <div className="">
            <h2 className={`${styles.h3} text-secondary2 `}>
              Key Features
            </h2>
            <div className="flex gap-2 md:gap-6 flex-wrap py-4">
